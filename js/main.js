...
6. SCROLL TO CONTENT
7. CONTENT APPEAR ANIMATION INIT / #ifest-details
8. CONTENT CLOSE INITI / #ifest-close-content
9. NEWSLETTER FORM INIT / #ifest-form
...
21. Preloader / .ifest-loading-screen
...
function contentToggle($this){
  ...
  var self    = $this,
    details   = $('#ifest-details'),
    content   = $('#ifest-animate-area'),
    close     = $('#ifest-close-content'),
    hide      = content.data('hide'),
    animation = details.data('animation');

  if ( self.hasClass('ifest-animate-open') ) {
    close.hide(0);
    self.removeClass('ifest-animate-open');
    ...
    self.addClass('ifest-animate-open');
...
--------------------------------------------------------------*/
$(document).on("click", ".ifest-animate-btn", function(e){
  e.preventDefault();
  var self    = $(this);
...
function clickToSlide($this){
  ...
  var self    = $this,
    content   = $('#ifest-details'),
    col   = $('.ifest-col-content'),
    close     = $('#ifest-close-content-slide'),
...
$(document).on("click", ".ifest-content-view", function(e){
  e.preventDefault();
  var self = $(this);
...
--------------------------------------------------------------*/
$(document).on("click", "#ifest-close-content", function(e){

  e.preventDefault();

  var self    = $(this),
    details   = $('#ifest-details'),
    btn       = $('.ifest-animate-btn'),
    content   = $('#ifest-animate-area'),
    close     = $('#ifest-close-content'),
    hide      = content.data('hide'),
    animation = details.data('animation');
    ...
    close.hide();
    btn.removeClass('ifest-animate-open');
    content.removeClass(hide).addClass(animation).addClass('mrs-active');
    details.removeClass(animation).addClass(hide);
  
});

$(document).on("click", "#ifest-close-content-slide, .ifest-col-content.mrs-default-content-off", function(e){

  e.preventDefault();

  var self    = $(this),
    details   = $('#ifest-details'),
    btn       = $('.ifest-content-view'),
    close     = $('#ifest-close-content-slide'),
    col   = $('.ifest-col-content'),
    animation = details.data('animation');
...
if ( $("#ifest-form").is_exist() ) {

  var mform = $("#ifest-form");
  mform.ajaxChimp({
  ...
  function callbackFunction (resp) {
      if (resp.result === 'success') {
        ...
        if ( $('.ifest-newsletter-header').length > 0 ) {
          $('.ifest-newsletter-header').removeClass('mform-header-animate').addClass('mfrom-header-animate-close');
        }
        setTimeout(function(){
          mform.removeClass('mform-success').removeClass('mform-submitting').removeClass('mform-animate');
          if ( $('.ifest-newsletter-header').length > 0 ) {
            $('.ifest-newsletter-header').removeClass('mfrom-header-animate-close').addClass('mfrom-header-animate');
          }
...
$(window).on("load resize",function(){
  ...
  if ( $(".ifest-main-content").is_exist() ) {

    var content = $(".ifest-main-content"),
        close   = $('#ifest-close-content');
...
/*--------------------------------------------------------------
  21. Preloader
--------------------------------------------------------------*/
$(".ifest-loading-screen").delay(200).fadeOut('slow');
